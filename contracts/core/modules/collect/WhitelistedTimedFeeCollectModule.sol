 ...
 * @param merkleRoot Merkle Root of whitelist.
 */
...
/**
 * @title WhitelistedTimedFeeCollectModule
 ...
 */
contract WhitelistedTimedFeeCollectModule is FeeModuleBase, FollowValidationModuleBase, ICollectModule {
     ...
     *      bytes32 merkleRoot: Merkle Root of whitelist.
     *
    ...
    function initializePublicationCollectModule(
        ...
        unchecked {
            ...
            (
                ...
                bool followerOnly,
                bytes32 merkleRoot
            ) = abi.decode(data, (uint256, address, address, uint16, bool, bytes32));
            if (
            ...
            _dataByPublicationByProfile[profileId][pubId].merkleRoot = merkleRoot;

            return abi.encode(amount, currency, recipient, referralFee, followerOnly, endTimestamp, merkleRoot);
        }
    }
    ...
    function processCollect(
        ...
        if (referrerProfileId == profileId) {
            _processCollect(collector, profileId, pubId, data[:64]);
        } else {
            _processCollectWithReferral(referrerProfileId, collector, profileId, pubId, data[:64]);
        }
    }
